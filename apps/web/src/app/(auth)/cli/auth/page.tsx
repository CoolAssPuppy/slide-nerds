...
function CliAuthContent() {
  ...
  const handleGitHubLogin = async () => {
    setError(null)
    // For OAuth, we redirect to GitHub, then come back to /auth/callback
    // which sets the session. Then the user lands back here and the
    // useEffect picks up the session.
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'github',
      options: {
        redirectTo: `${window.location.origin}/cli/auth?callback=${encodeURIComponent(callbackUrl ?? '')}`,
      },
    })
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 h-10 rounded-[var(--n-radius-md)] border border-[var(--border)] bg-[var(--card)] text-sm font-medium hover:bg-[var(--accent)] transition-colors"
        >
